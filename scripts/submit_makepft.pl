...
# This perl script submits a batch job to create PFT maps
...
# generate one-file-per-pft (manyfiles=1) or single files with all pft's inside (manyfiles=0)
my $manyfiles = 0;
...
if (! -d $pftdir) {
    if (`mkdir $pftdir` ne "") {
        die "Cannot create $pftdir \n";
    }
    print "Created directory: $pftdir \n";
}

my $batchdir = $pftdir."\/batch";
if (! -d $batchdir) {
...
#	for (my $xproc=1;$xproc<=$nxprocs;$xproc++) {
		...
		print OUT <<EOF;
.r $exename
$exename,basedir="$projdir",pftcase="$pftcase",manyfiles=$manyfiles, \$
...
EOF
	    ...
	    if ($install == 1) {
		print OUT <<EOF;
cd $idldir; $runcmd idl -queue -rt="$exename.sav" -args basedir="$projdir" pftcase="$pftcase" manyfiles=$manyfiles lonmin0=$sitexmin[$s]d lonmax0=$sitexmax[$s]d latmin0=$siteymin[$s]d latmax0=$siteymax[$s]d dlon0=$sitedeltax[$s]d dlat0=$sitedeltay[$s]d region="$sitename[$s]" xproc=$xproc yproc=$yproc nxprocs=$nxprocs nyprocs=$nyprocs
EOF
	    } else {
		print OUT <<EOF;
...
EOF
	    ...
	    if (`chmod +x $jobfile` ne "") {
		die "Cannot make $jobfile executable \n";
	    }
	    ;
