...
;
; :Params:
;   x: in, out, required, type=float
;      the important input variable
;
; :Keywords:
;   y: in, optional, type="lonarr(3,ndims) or string"
;      provides a keyword input variable
;
; :Returns: 
; 
; :Uses:
...
;  prof_start = systime(1)
  ...
  if ~keyword_set(autoderiv) then begin
      mperr = 0
      fjac = intarr(nall)
...
;  prof_start = systime(1)
  ...
  for j = 0L, minmn-1 do begin
      if ~keyword_set(pivot) then goto, HOUSE1
...
function mpfit, fcn, xall, FUNCTARGS=fcnargs, SCALE_FCN=scalfcn, $
  ...
  ;; gradients.
  if damp NE 0 AND ~keyword_set(autoderiv) then begin
  ...
  if n_elements(state) NE 0 AND ~keyword_set(extinit) then begin
      szst = size(state)
...
;  prof_start = systime(1)
  ...
  if (iter EQ 1) then begin
      ...
      if ~keyword_set(rescale) OR (n_elements(diag) LT n) then begin
          diag = wa2
...
TERMINATE:
  ...
  if info GT 0 AND ~keyword_set(nocovar) $
