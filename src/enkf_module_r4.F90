...
subroutine enkf(HA, obsdat, obserr, mode, X, A)
  ...
  ! arguments
  real(kind=4), intent(inout) :: HA(:,:)  ! model states and parameters in observation space
  real(kind=4), intent(in) :: obsdat(:)   ! observation vector
  real(kind=4), intent(in) :: obserr(:)   ! observation variance vector
  ...
  real(kind=4), intent(out), optional :: X(:,:)     ! ensemble covariance matrix for updating A externally
  real(kind=4), intent(inout), optional :: A(:,:)   ! model state and parameter matrix for internal update
  ...
  real(kind=4) :: observation_variance
  real(kind=4), allocatable :: HAmean(:)
  real(kind=4), allocatable :: observation_mean(:)

  ! internal EnKF arrays
  real(kind=4), allocatable :: eig(:)
  real(kind=4), allocatable :: W(:,:)
  real(kind=4), allocatable :: X2(:,:)
  real(kind=4), allocatable :: X3(:,:)
  real(kind=4), allocatable :: Reps(:,:)
  real(kind=4), allocatable :: X5(:,:)      ! Ensemble covariance matrix (coefficients)
  real(kind=4), allocatable :: E(:,:)       ! observation error covariance matrix
  real(kind=4), allocatable :: IM(:,:)      ! innovation matrix
  real(kind=4), allocatable :: D(:,:)       ! perturbed observation matrix
  real(kind=4), allocatable :: R(:,:)       ! matrix holding error covariance matrix for observations R 
  real(kind=4), allocatable :: innov(:)     ! vector holding d-H*mean(A)

  ! parameters
  logical, parameter :: verbose = .false.           ! Printing some diagnostic output
  real(kind=4), parameter :: truncation = 0.99      ! The ratio of variance retained in pseudo inversion (0.99)  
  ...
  observation_variance=sqrt(observation_variance/real(nrens*nrobs))
  ...
  if (nrobs == 1) then
     ...
     allocate(W(1,1))
     allocate(eig(1))
     eig(1)=dot_product(HA(1,:),HA(1,:))+real(nrens-1)*R(1,1)
     eig(1)=1.0/eig(1)
     W(1,1)=1.0
...
!        call sgemm('n','t',ndim,nrobs,nrens,1.0,A,ndim,HA,nrobs,0.0,Reps,ndim)
!     else
        if (verbose) write(*,'(A)') 'analysis: X5 approach is used'
        !        X5=matmul(transpose(HA),X3)
        call sgemm('t','n',nrens,nrens,nrobs,1.0,HA,nrobs,X3,nrobs,0.0,X5,nrens)
        deallocate(X3)
        do i=1,nrens
           X5(i,i)=X5(i,i)+1.0
        enddo
...
subroutine kernel_smooth(A)
  ...
  ! local variables
  real(kind=4) :: ensemble_mean, ensemble_variance
  real(kind=4), allocatable :: random_ensemble(:)
  integer :: nrens
  real(kind=4), parameter :: alpha = 0.995 ! shrinkage factor
...
subroutine kernel_inflate(A,var,alpha)
  ...
  real(kind=4), intent(in) :: alpha ! minimum variance factor

  ! local variables
  real(kind=4) :: ensemble_mean, ensemble_variance
  real(kind=4) :: varscale
  integer :: nrens
  ...
  varscale = max(var/max(ensemble_variance,1.e-6)*alpha,1.0)
...
subroutine kernel_deflate(A,var,alpha)
  ...
  ! arguments
  real(kind=4), intent(inout) :: A(:)
  real(kind=4), intent(in) :: var ! prior ensemble variance
  real(kind=4), intent(in) :: alpha ! minimum variance factor
  ...
  varscale = min(var/max(ensemble_variance,1.e-6)*alpha,1.0)
...
subroutine kernel_rescale(A,varscale)
  ...
  ! arguments
  real(kind=4), intent(inout) :: A(:)
  real(kind=4), intent(in) :: varscale ! variance scale factor
...
subroutine kernel_limit(A,A_min,A_max)
  ...
  ! arguments
  real(kind=4), intent(inout) :: A(:)
  real(kind=4), intent(in) :: A_min
  real(kind=4), intent(in) :: A_max
...
subroutine multa(A, X, ndim, nrens, iblkmax)
  ...
  real(kind=4), intent(in)    :: X(nrens,nrens)
  real(kind=4), intent(inout) :: A(ndim,nrens)

  ! local variables
  real(kind=4) :: v(iblkmax,nrens)  ! Automatic work array 
  integer :: ia,ib
...
subroutine random(work1,n)
  ...
  ! arguments
  integer, intent(in) :: n
  real(kind=4), intent(out) :: work1(n)

  ! local variables
  real(kind=4), allocatable :: work2(:)
  real(kind=4), parameter   ::  pi=3.141592653589
  ...
  work1 = sqrt(-2.0*log(work1+tiny(0.0)))*cos(2.0*pi*work2)
...
subroutine randrot(Q,nrens)
  ...
  ! arguments
  integer, intent(in)  :: nrens
  real(kind=4),    intent(out) :: Q(nrens,nrens)
  
  ! local variables
  real(kind=4) ::  A(nrens,nrens)
  real(kind=4) :: B(nrens,nrens)
  real(kind=4) :: sigma(nrens)
  real(kind=4) :: work(10*nrens)
  real(kind=4), parameter :: pi=3.14159253589
  integer :: ierr
  
  call random_number(B)
  call random_number(A)
  Q = sqrt(-2.0*log(A+tiny(0.0))) * cos(2.0*pi*B)

!$OMP END CRITICAL
! QR factorization
  call sgeqrf(nrens, nrens, Q, nrens, sigma, work, 10*nrens, ierr )
  if (ierr /= 0) write(*,'(A,I6)')  'randrot: sgeqrf ierr=',ierr
  
! Construction of Q
  call sorgqr(nrens, nrens, nrens, Q, nrens, sigma, work, 10*nrens, ierr )
  if (ierr /= 0) write(*,'(A,I6)')  'randrot: sorgqr ierr=',ierr
!$OMP END CRITICAL
...
subroutine lowrankE(S,E,nrobs,nrens,nrmin,W,eig,truncation)
  ...
  real(kind=4),    intent(in)  :: S(nrobs,nrens)
  real(kind=4),    intent(in)  :: E(nrobs,nrens)
  real(kind=4),    intent(out) :: W(nrobs,nrmin)
  real(kind=4),    intent(out) :: eig(nrmin)
  real(kind=4),    intent(in)  :: truncation
  
  ! local variables
  real(kind=4) :: U0(nrobs,nrmin)
  real(kind=4) :: sig0(nrmin)
  real(kind=4) :: X0(nrmin,nrens)
  real(kind=4) :: U1(nrmin,nrmin)
  real(kind=4) :: VT1(1,1)
  integer :: i,j  
  integer :: lwork
  integer :: ierr
  ...
  deallocate(work)
  if (ierr /= 0) then
     write(*,'(A,I6)') '(lowrankE): ierr from call sgesvd 1= ',ierr
     stop
  endif
...
subroutine eigC(R,nrobs,Z,eig)
  ...
  ! arguments
  integer, intent(in) :: nrobs
  real(kind=4), intent(in)    :: R(nrobs,nrobs)
  real(kind=4), intent(out) :: Z(nrobs,nrobs)
  real(kind=4), intent(out)   :: eig(nrobs)

  ! local variables
  real(kind=4) ::  RR(nrobs,nrobs)
  real(kind=4) :: fwork(8*nrobs)  
  integer :: iwork(5*nrobs)
  integer :: ifail(nrobs)
  real(kind=4) :: abstol,ddum
  integer :: idum,neig,ierr
  real(kind=4), external :: SLAMCH
...
!     enddo
!  enddo
!  call spev(21,ap,eig,Z,nrobs,nrobs,fwork,2*nrobs)
!  deallocate(ap)
!#else
  abstol=2.0*SLAMCH('S')
  RR=R
  call ssyevx('V', 'A', 'U', nrobs, RR, nrobs, ddum, ddum, idum, idum, abstol, &
...
subroutine eigsign(eig,nrobs,truncation)
  ...
  ! arguments
  integer, intent(in)    :: nrobs
  real(kind=4),    intent(inout) :: eig(nrobs)
  real(kind=4),    intent(in)    :: truncation
  
  ! local variables
  integer :: i,nrsigma
  real(kind=4) :: sigsum,sigsum1
...
subroutine genX2(nrens,nrobs,i_dim,S,W,eig,X2)
  ...
  real(kind=4), intent(in)    :: W(i_dim,nrens)
  real(kind=4), intent(in)    :: S(nrobs,nrens)
  real(kind=4), intent(in)    :: eig(i_dim)
  real(kind=4), intent(out)   :: X2(i_dim,nrens)
  integer :: i,j
  
  call sgemm('t','n',i_dim,nrens,nrobs,1.0,W,nrobs, S,nrobs, 0.0,X2,i_dim)
...
subroutine genX3(nrens,nrobs,nrmin,eig,W,D,X3)
  ...
  integer, intent(in) :: nrobs
  integer, intent(in) :: nrmin
  real(kind=4),    intent(in) :: eig(nrmin)
  real(kind=4),    intent(in) :: W(nrobs,nrmin)
  real(kind=4),    intent(in) :: D(nrobs,nrens)
  real(kind=4),    intent(out) :: X3(nrobs,nrmin)
  
  ! local variables
  real(kind=4) :: X1(nrmin,nrobs)
  real(kind=4) :: X2(nrmin,nrens)
  integer :: i,j
...
subroutine meanX5(nrens,nrobs,nrmin,S,W,eig,innov1,X5)
  ...
  integer, intent(in) :: nrobs
  integer, intent(in) :: nrmin
  real(kind=4), intent(in)    :: W(nrmin,nrmin)
  real(kind=4), intent(in)    :: S(nrobs,nrens)
  real(kind=4), intent(in)    :: eig(nrmin)
  real(kind=4), intent(in)    :: innov1(nrobs)
  real(kind=4), intent(out)   :: X5(nrens,nrens)
  
  ! local variables
  real(kind=4) :: y1(nrmin)
  real(kind=4) :: y2(nrmin)
  real(kind=4) :: y3(nrobs)
  real(kind=4) :: y4(nrens) 
  integer :: i
  ...
  if (nrobs==1) then
     ...
     call sgemv('t',nrobs,nrmin,1.0,W,nrobs,innov1,1,0.0,y1 ,1)
     y2=eig*y1  
...
subroutine X5sqrt(X2,nrobs,nrens,nrmin,X5,mode)
  ...
  real(kind=4), intent(in)    :: X2(nrmin,nrens)
  real(kind=4), intent(inout) :: X5(nrens,nrens)
  integer, intent(in) :: mode
  
  ! local variables
  real(kind=4) :: X3(nrens,nrens)
  real(kind=4) :: X33(nrens,nrens)
  real(kind=4) :: X4(nrens,nrens)
  real(kind=4) :: IenN(nrens,nrens)    
  real(kind=4) :: U(nrmin,1),sig(nrmin),VT(nrens,nrens)
  integer :: i,j,lwork,ierr
  real(kind=4), allocatable, dimension(:)   :: work,isigma
  ...
  deallocate(work)
  if (ierr /= 0) then
     write(*,'(A,I6)') 'X5sqrt: ierr from call sgesvd = ',ierr
     stop
  endif
  ...
  isigma=1.0
  do i=1,nrmin
     if ( sig(i) > 1.0 ) write(*,'(A,I6,G14.6)') 'X5sqrt: WARNING (m_X5sqrt): sig > 1',i,sig(i)
...
subroutine lowrankCinv(S,R,nrobs,nrens,nrmin,W,eig,truncation)
  ...
  real(kind=4) :: U0(nrobs,nrmin)
  real(kind=4) :: sig0(nrmin)
  real(kind=4) :: B(nrmin,nrmin)
  real(kind=4) :: Z(nrmin,nrmin)
  integer :: i,j
...
subroutine lowrankCee(B,nrmin,nrobs,nrens,R,U0,sig0)
  ...
  integer, intent(in) :: nrobs
  integer, intent(in) :: nrens
  real(kind=4), intent(inout) :: B(nrmin,nrmin)
  real(kind=4), intent(in)    :: R(nrobs,nrobs)
  real(kind=4), intent(in)    :: U0(nrobs,nrmin)
  real(kind=4), intent(in)    :: sig0(nrmin)

  ! local variables
  real(kind=4) :: X0(nrmin,nrobs)
  integer ::  i,j
...
subroutine svdS(S, nrobs,nrens,nrmin,U0,sig0,truncation)
  ...
  integer, intent(in)  :: nrmin
  real(kind=4),    intent(in)  :: S(nrobs,nrens)
  real(kind=4),    intent(out) :: sig0(nrmin)
  real(kind=4),    intent(inout)  :: U0(nrobs,nrmin)
  real(kind=4),    intent(in)  :: truncation

  ! local variables
  real(kind=4) :: S0(nrobs,nrens)
  real(kind=4) :: VT0(1,1)
  integer :: ierr
  integer :: lwork
  integer :: nrsigma,i
  ...
  ! important (need a local and variable array for sgesv)
  S0=S
  ...
  deallocate(work)
  if (ierr /= 0) then
     write(*,'(A,I6)') 'svdS: ierr from call sgesvd 0= ',ierr
     stop
  endif
...
subroutine mean_preserving_rotation(Up,nrens)
  ...
  ! arguments
  integer, intent(in)    :: nrens
  real(kind=4), intent(out) :: Up(nrens,nrens)

  ! local variables
  real(kind=4) :: B(nrens,nrens)
  real(kind=4) :: Q(nrens,nrens)
  real(kind=4) :: R(nrens,nrens)
  real(kind=4) :: U(nrens-1,nrens-1)
  real(kind=4) :: Upb(nrens,nrens)
  integer :: j,k
